...
**Reason**: 引入 SqliteSaver 后，LangGraph checkpointer 按 `thread_id` 自动加载与落盘 messages，无需也不应再由 CLI 维护完整 state。手动 rebuild 路径与 checkpointer 自动管理存在双写风险，已废弃。

**Migration**: CLI 每轮仅传 `{"messages": [HumanMessage(user_input)]}` 给 `agent.astream`；历史依赖 checkpointer 恢复。多轮上下文连续性由 `leader-persistence` 能力的"多轮上下文连续"Scenario 保证。
